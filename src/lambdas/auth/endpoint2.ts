...
type Event = {
  httpMethod: string
  queryStringParameters: { email: string; password: string }
...
export class Page {
  ...
  async loginUser(email: string, pass: string, client: DynamoDBDocumentClient) {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify(
        'Could not find an account with that email. Do you want to register?'
      )
    }
  }
}
...
export const handler = async (event: Event) => {
  ...
  if (event.httpMethod === 'POST') {
    ...
    const correctEmailFormat = new RegExp(
