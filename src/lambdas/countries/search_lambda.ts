...
async function scanCountryIndicators(
  filters: APIGatewayProxyEventQueryStringParameters,
  ...
  try {
    ...
    if (countries && countries.length > 0) {
      for (const country of countries) {
        ...
        const countryData = await scanCountryIndicators(
          countryParams,
          docClient
        )
        data.push(countryData)
      }
      const searchData: Search = await createSearch(
        accessToken,
        parameters,
        countries
      )
