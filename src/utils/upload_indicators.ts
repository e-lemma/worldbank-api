...
async function readFile(fileName: string) {
    const filePath = path.resolve(__dirname, `../../../src/utils/archive/${fileName}`)

    const countryIndicators: CountryIndicator[] = []
    const stream = fs.createReadStream(filePath).pipe(
        parse({ delimiter: ",", from_line: 1, columns: true })
    )


    for await (const row of stream) {
        const {CountryName, CountryCode, IndicatorName, IndicatorCode, Year, Value } = row
...
async function uploadIndicators(indicators: CountryIndicator[], docClient: DynamoDBClient) {
    const chunkSize = 25
